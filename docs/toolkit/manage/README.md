...
Cluster scoped resources can be provided via the `--clusterScopedResource`/`-c` argument, which must match one of the available [API resources](../list/README.md#apiresources) for the given cluster in "{group}/{version}/{kind}" format.  A single cluster scoped resource should be provided per `--clusterScopedResource`/`-c` flag, with an optional additional label selector.

```text
$ actoolkit manage app cloudbees-core cloudbees-core 690deba1-bc57-4771-ab72-88758cab2afd \
    -c admissionregistration.k8s.io/v1/ValidatingWebhookConfiguration \
    -c rbac.authorization.k8s.io/v1/ClusterRole app.kubernetes.io/instance=cloudbees-core \
    -c rbac.authorization.k8s.io/v1/ClusterRoleBinding app.kubernetes.io/instance=cloudbees-core
