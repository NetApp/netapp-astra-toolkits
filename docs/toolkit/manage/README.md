...
```text
$ actoolkit manage -h
usage: actoolkit manage [-h] {app,bucket,cloud,cluster,ldap} ...
...
objectType:
  {app,bucket,cloud,cluster,ldap}
    ...
    ldap                manage an LDAP(S) server for remote authentication
```
...
```text
$ actoolkit manage ldap 10.193.246.200 389 -u service-account@astra-example.com -p SA-Password --userBaseDN OU=e2e,DC=astra-example,DC=com --groupBaseDN OU=e2e,DC=astra-example,DC=com
